...
public class PostLogProfilerProcessor {
	 ...
	 *
	 * @param args
	 *            command line arguments. Expects one argument, the name of the file
	 *            to post process.
	 * @throws Exception
	...
	public PostLogProfilerProcessor(String filename, String fomattFileName) throws Exception {

		StringBuilder out = new StringBuilder();

		// 파일 객체 생성
		File file = new File(fomattFileName);
		...
		out.append("processed " + lineNo + " lines.").append("\n");

		f.close();

		// display report to stdout

		out.append("Number of sql statements:  " + totalSql).append("\n");
		out.append("Total number of msec    :  " + totalMsec).append("\n");
		if (totalMsec > 0) {
			out.append("Average msec/statement  :  " + totalSql / totalMsec).append("\n");
		}

		int flaggedSqlStmts = flaggedSql.size();

		if (flaggedSqlStmts > 0) {
			out.append("Sql statements that took more than " + threshold + " msec were flagged.").append("\n");
			out.append("Flagged sql statements              :  " + flaggedSqlStmts).append("\n");
			out.append("Flagged sql Total number of msec    :  " + flaggedSqlTotalMsec).append("\n");
			out.append("Flagged sql Average msec/statement  :  " + flaggedSqlTotalMsec / flaggedSqlStmts).append("\n");

			out.append("sorting...").append("\n");
			...
			out.append("top " + topOffenderCount + " offender" + (topOffenderCount == 1 ? "" : "s") + ":").append("\n");

			ProfiledSql p;

			for (int i = 0; i < topOffenderCount; i++) {
				p = (ProfiledSql) flaggedSqlArray[i];
				out.append(Utilities.rightJustify(execTimeSize, "" + p.getExecTime()) + " " + p.getSql()).append("\n");
			}
		}
	...
	private class ProfiledSql implements Comparable {
		 ...
		 *
		 * @param o
		 *            ProfiledSql Object to compare to this ProfiledSql. Must not be
		 *            null.
		 */
