...
public class PageNavigator {
	...
	public PageNavigator(Map<String, Object> param, int totalCount) throws Exception {
		...
		String rowSize = (String) param.get("rowSize");
		if (StringUtil.isNotEmpty(rowSize)) {
			searchVO.setRowSize(Integer.parseInt(rowSize));
		}
	...
	public String getFirstPage(String imageTag) {
		...
		if (this.pageNo > 0) {
			strTag = "<a href='#none' class='btn btn_first' onclick='javascript:" + this.functionName
					+ "(1); return false;'>" + imageTag + "</a>";
		}
		return strTag;
	}
	...
	public String getLastPage(String imageTag) {
		...
		if (this.pageNo < this.lastPageNo) {
			strTag = "<a href='#none' class='btn btn_last' onclick='javascript:" + this.functionName + "("
					+ this.lastPageNo + "); return false;'>" + imageTag + "</a>";
		} else {
			strTag = "<a href='#none' class='btn btn_last'>" + imageTag + "</a>";
		}
		return strTag;
	}
	...
	public String getNextBlock(String imageTag) {
		...
		if (this.endPageNo < this.lastPageNo) {
			...
			strTag = "<a href='#none' class='btn btn_next3' onclick='javascript:"
					+ this.functionName + "(" + nextBlockPageNo + ", 1); return false;'><li style='border-right: none;'>" + imageTag + "</li></a>";
		} else {
			strTag = "<a href='#none' class='btn btn_next3'><li style='border-right: none;'>" + imageTag + "</li></a>";
		}
		return strTag + "</ul>";
	}
	...
	public String getPageList(String delim, boolean type) {
		...
		for (int i = this.startPageNo; i <= this.endPageNo; ++i) {
			...
			if (i == this.pageNo) {
				strTag = strTag + "<li class='select'>" + pageMark + "</li>";
			} else {
				strTag = strTag + "<a href='#none' onclick='javascript:" + this.functionName + "(" + i
						+ "); return false;'><li>" + pageMark + "</li></a>";
			}
	...
	public String getPrevBlock(String imageTag) {
		...
		if (this.pageNo > this.blockSize) {
			strTag = "<a href='#none' class='btn btn_prev3' onclick='javascript:" + this.functionName + "("
					+ prevBlockPage + "); return false;'><li>" + imageTag + "</li></a>";
		} else {
			strTag = "<a href='#none' class='btn btn_prev3'><li>" + imageTag + "</li></a>";
		}
		return "<ul>" + strTag;
	}
