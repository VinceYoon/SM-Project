		...
		<section class="type01">
			...
			<div class="group01">
				...
				<div class="list_detail">
					<!-- s: form -->
					<form id="frmReg" name="frmReg" method ="post" action="/schMng/schMngEditFormAjax">
						<input type="hidden" id=scheduleMngSeq name="scheduleMngSeq" value="${schMng.scheduleMngSeq }"/>
						<input type="hidden" id="reportSeq" name="reportSeq" value="${schMng.reportSeq }"/>
						<input type="hidden" id="confirmYn" name="confirmYn" value="${schMng.confirmYn }"/>
						<input type="hidden" id="mngYn" name="mngYn" value="${schMng.mngYn }"/>
						<ul>
							<li>
								...
								<dd>
									<c:forEach var="item" items="${schGubunList }" varStatus="status">
										...
										<input type="radio" id="${item.code}"  name="scheduleGubun" value="${item.code}" <c:if test="${item.code eq schMng.scheduleGubun}">checked="checked"</c:if>> 
										<label for="${item.code}"><span></span>${item.codeNm}</label>
										</div>
									</c:forEach>
									<div class="selectbox" style="width:55px; float:left; margin:-2px 0px 0px 18px;"> 
										...
										<select id="colorCd" name="colorCd"> 
											<c:forEach var="item" items="${colorList }" varStatus="status">
												<option value="${item.code }" style="font-size: 20px; color:#${item.code }" <c:if test="${item.code eq schMng.colorCd}">selected="selected"</c:if>>●</option> 
											</c:forEach>
										</select>
									</div>
								</dd>
							...
							<li class="company">
								...
								<dd class="pr20">
									<input type="text" id="companyNm" name="companyNm"style="width:220px;" placeholder="업체명을 입력해주세요." value="${schMng.companyNm }"/>
									<span style="color: #999;" id="companyAddr">${schMng.companyAddr }</span>
									<input type="hidden" id="companySeq" name="companySeq" value="${schMng.companySeq }"/>
							...
							<li>
								...
								<dd class="pr20">
									<input type="text" id="mngNm" name="mngNm" style="width:220px;" placeholder="이름을 입력해주세요." value="${schMng.mngNm }"/>
									<input type="hidden"id="mngId" name="mngId" value="${schMng.mngId }"/>
							...
							<li>
								...
								<dd>
									...
									<input type="text" id="startTime" name="startTime" style="width:80px; text-align: center; vertical-align: middle;" maxlength="5">
									 &nbsp;~&nbsp;
									<input type="text" id="endDt" name="endDt" class="datepicker" style="width:110px;" readonly="readonly"  />
									<input type="text" id="endTime" name="endTime" style="width:80px; text-align: center; vertical-align: middle;" maxlength="5">
								</dd>
							</li>
							<li>
								<dt>동행자  <span class="font05">*</span></dt>
								<dd>
							...
							<!-- 선택된 동행자가 있을경우 출력 -->
							<li id="selectPartnerDetailLi" <c:if test="${empty schMngUser}">style="display:none;"</c:if>>
								<dt>선택된 동행자</dt>
								<dd class="pr20 select_partner_detail" id="selectPartnerDetailDd">
									<c:forEach var="item" items="${schMngUser }" varStatus="status">
										<button type="button" style="margin-right:5px;" class="btn_table_blue" id="staffButton_${item.userId }" >
											<input type="hidden" name="staffId" value="${item.userId }">
											${item.userNm }&nbsp;${item.positionNm }&nbsp;
											<img src="${STATIC_COMMON_URL}/images/icon_del_blue.png" onclick="delUser('${item.userId }')">
										</button>	
									</c:forEach>
								</dd>
							</li>
							<li>
								<dt>특이사항</dt>
								<dd>
									<textarea rows="5" cols="62" name="scheduleMemo">${schMng.scheduleMemo }</textarea>
								</dd>
							</li>
						</ul>
					</form>	
				...
				<div class="btn_box_con">
					<c:choose>
						<c:when test="${schMng.confirmYn eq 'Y'}">
							<button class="btn_admin_blue" onClick="changeConfirm();">완료 취소</button>
						</c:when>
						<c:otherwise>
							<button class="btn_admin_blue" onClick="changeConfirm();">완료</button>
						</c:otherwise>
					</c:choose>
					<button type="button" class="btn_admin_red" onClick="editForm();">수정</button> 
					<button type="button" class="btn_admin_navi"onclick="delForm()">삭제</button>
					<button type="button" class="btn_admin_gray" onClick="cancelForm();">취소</button>
				</div>
			</div>
			<!-- e: group01 -->	
		</section>
...
<script type="text/javascript">
	...
	$(function(){
		...
		initDatepicker();
		// 컬러 데이터 값
		$('#color').css('color','#${schMng.colorCd}');
		// 날짜 시간 데이터 값 세팅
		setDateTime();
		// 업체 조회
		var xhr;
...
// 							var name = list[i].companyNm + " / " + list[i].codeSiNm + " / " + list[i].codeGuNm;
	...
	function setDateTime(){
		var startDt = '${schMng.startDt}'.substring(0,4) + "-" + '${schMng.startDt}'.substring(4,6) + "-" + '${schMng.startDt}'.substring(6,8);
		var endDt = '${schMng.endDt}'.substring(0,4) + "-" + '${schMng.endDt}'.substring(4,6) + "-" + '${schMng.endDt}'.substring(6,8);
		var startTime = '${schMng.startTime}'.substring(0,2) + ":" + '${schMng.startTime}'.substring(2,4);
		var endTime = '${schMng.endTime}'.substring(0,2) + ":" + '${schMng.endTime}'.substring(2,4);
		$('#startDt').val(startDt);
		$('#endDt').val(endDt);
	...
	function editForm() {
		if($('#confirmYn').val() == 'Y'){
			alert("이미 완료 처리된 일정으로 수정이 불가합니다.");
			return false;
		}
	...
	function delForm(){
		if($('#confirmYn').val() == 'Y'){
			alert("이미 완료 처리된 일정으로 삭제가 불가합니다.");
			return false;
		}
		...
		var scheduleMngSeq = $('#scheduleMngSeq').val();
		$.ajax({
			url: '/schMng/schMngDelFormAjax',
			dataType: 'json',	
			data: {'scheduleMngSeq': scheduleMngSeq },
			type: 'POST',
			async : false,
			success: function(data) {
				location.href = '/schMng/schMngList';
	...
	function changeConfirm(){
		if($('#mngYn').val() == 'Y'){
			alert("최종 완료 처리된 일정으로 완료취소가 불가합니다.");
			return false;
		}
		var confirmYn = $('#confirmYn').val();
		var scheduleMngSeq = $('#scheduleMngSeq').val();
		$.ajax({
			url: '/schMng/changeConfirmFormAjax',
			dataType: 'json',	
			data: {'scheduleMngSeq': scheduleMngSeq, 'confirmYn': confirmYn },
			type: 'POST',
			async : false,
			success: function(data) {
				location.href = '/schMng/schMngList';
